...
def createCalendar(request):
    if request.method == "POST":
        ...
        user_id = request.POST['user_id']
        calendar = Calendar(memoryname=memoryname, memorycontent=memorycontent, memorydate=memorydate, createdBy=user_id)
        calendar.save()
...
def createTree(request):
    if request.method == "POST":
        ...
        user_id = request.POST['user_id']
        tree = LoveTree(treename=treename, createdBy=user_id)
        tree.save()
        response_json = {'status':'success', 'message':'种树成功'}
        return JsonResponse(response_json) 

def editTree(request):
    if request.method == "POST":
        id = request.POST['id']
        tree = LoveTree.objects.get(id=id)
        tree.treename = request.POST['treename']
...
def addWater(request):
    if request.method == "GET":
        ...
        if int(tree.createdBy) != int(request.POST['user_id']):
            ...
            tree.add_water()
            tree.save()
            response_json = {'status':'success', 'message':'浇水成功'}
        return JsonResponse(response_json)
